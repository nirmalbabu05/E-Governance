...
* 💬 **Fuzzy-Logic NLP Virtual Assistant:** An integrated chatbot powered by the Levenshtein Distance algorithm that tolerates typos (e.g., "gerbage" -> "garbage") to help citizens track statuses 24/7.
* ⚡ **Event-Driven Administrative Dashboard:** Powered by a NoSQL backend, providing instant WebSocket updates without page reloads. Features predictive analytics to monitor resolution velocity and workforce performance.
...
* **TensorFlow.js (`@tensorflow/tfjs`):** Client-side inference via quantized MobileNet V2.
* **Custom NLP Engine:** Levenshtein Distance Algorithm for fuzzy string matching.
* **Multimodal Fusion Logic:** JavaScript algorithms linking AI vision vectors with human text context.

**Backend & Database**
* **Google Firebase Realtime Database:** NoSQL JSON tree for blazing-fast, event-driven data sync.
* **Firebase Authentication:** Secure login and session management via JWT tokens.

**Hardware APIs & Cryptography**
* **HTML5 Geolocation API:** Forced high-accuracy mode for hardware-locked GPS tracking.
* **WebRTC / MediaDevices API:** Direct camera hardware access to enforce live-capture protocols.
